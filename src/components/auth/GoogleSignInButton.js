...
export function GoogleSignInButton({
  ...
  const handleGoogleLogin = () => {
    ...
    try {
      ...
      if (!window.google) {
        const errorMsg =
          "Google sign in not available. Please refresh the page.";
  ...
  return (
    <Button
      ...
      disabled={loading}
      className={`w-50 flex items-center cursor-pointer justify-center gap-3 h-11 border-2 border-gray-300 bg-white hover:bg-gray-50 text-gray-700 hover:text-gray-900 transition-all duration-200 hover:shadow-lg ${className}`}
      type="button"
    >
      {loading ? (
        <>
